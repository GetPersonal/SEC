...
public class BizantineServer extends UnicastRemoteObject implements IBlockServer {
	...
	private boolean primeirar= false;
	private boolean segundar= false;
	
		
	public BizantineServer() throws RemoteException {
		super();
		...
		if(hashBlocks.containsKey(id)) {
			...
			if(keyBlocks.containsKey(id)) {
				...
				if(id.equals(keyId)) {		
					...
					if(primeirar){
					return new MessageType(rts, block.key, block.writeTS.valueOf(0), block.block, block.signature );
					}
					primeirar=true;
	...
	public static void main(String[] args) throws Exception {
		BizantineServer server = new BizantineServer();
		Registry reg = null;
		try{
